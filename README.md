...
of object** — machine-checkable answers to _"is this build safe to release?"_ —
and belong in the same gating architecture. Neither is a report to read; both are
verdicts that block.
...
gate](https://github.com/qasimmahmood95/deploy-gates/actions/runs/29649076092)),
and a build that honors the contract perfectly but regresses latency ([blocked by
the k6 gate](https://github.com/qasimmahmood95/deploy-gates/actions/runs/29649126424)).
...
1. `npm run test:pact` — consumer test generates the contract
2. `npm run pact:publish` — contract goes to the self-hosted broker, versioned
   by commit + branch
3. `npm run pact:verify` — the provider is started in-process and verified
   against the contract; the result is published back
4. `npm run pact:can-i-deploy` — **the verdict**: green only if this exact
...
local-only dummy credentials — deliberately public, allowlisted string-by-string
for the secret scan. See [ADR-0002](docs/adr/0002-self-hosted-broker-vs-pactflow.md)
for why self-hosted instead of PactFlow.

### Performance gate (k6)

Two scenarios in [`k6/load.js`](k6/load.js) run against the provider:

- **steady** — ramping to sustained load; carries the primary SLO:
  `p95 < 250ms`, error rate `< 1%`, and 100% response-correctness checks
- **spike** — a sharp burst; a looser resilience bound (`p95 < 500ms`, errors
  `< 5%`) so runner noise can't flake the steady SLO
...
succeeded — scheduler-enforced, not script-promised. A `gate status summary` job
runs `if: always()` and writes both verdicts side by side into the run's job
summary, so a red run shows exactly which gate blocked. See
...
To enforce on `main`, mark these as required status checks (Settings → Branches):
`compatibility gate (pact)` · `performance gate (k6)` · `lint / typecheck /
test` · `secret scan (gitleaks)`.
...
| ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| The change              | provider renames `accountId` → `id`, updates its own tests to match                                                                                                   | provider gains ~350ms latency on data endpoints                                                                                                                         |
...
| Red run                 | [actions/runs/29649076092](https://github.com/qasimmahmood95/deploy-gates/actions/runs/29649076092)                                                                   | [actions/runs/29649126424](https://github.com/qasimmahmood95/deploy-gates/actions/runs/29649126424)                                                                     |

The symmetry is the point: one build fails only the contract gate, the other
fails only the performance gate, and **either alone blocks the deploy**. Unit
tests catch neither — both defect branches pass `lint / typecheck / test`.
